...
st.title("XGBoost 분석")
st.write("이 페이지에서는 XGBoost 모델 성능 분석을 보여줍니다.\n")
...
- 혼동 행렬(Confusion Matrix)을 보면:
    ...
    - 거짓 음성(FN): 35건 (이탈하지 않을 것으로 예측했으나 실제로 이탈함)
    - 진짜 양성(TP): 167건 (이탈할 것으로 예측했고 실제로 이탈함)

- 모델 성능 지표:
    - 정확도 93.37%: 전체 예측 중 약 92%가 정확함
    - 정밀도 90.27%: 이탈 예측 중 실제 이탈한 비율
    - F1 점수 86.30%: 정밀도와 재현율의 조화평균으로 양호한 수준
...
- AUC 값이 0.98으로 매우 높아, 모델이 98%의 확률로 양성과 음성을 구분할 수 있음을 보여줌. -> 모델의 분류 성능이 뛰어나며 신뢰할 수 있는 예측을 제공한다는 것을 의미.

#### **최종 결론**:
- 모델의 예측 성능이 전반적으로 우수하며, 특히 93.37%의 높은 정확도를 보임
